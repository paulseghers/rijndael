...
unsigned char** genLambdaset(){
  unsigned char* bytes1to15 = (unsigned char*) malloc(15*sizeof(unsigned char));
  for (size_t i=0; i<15; i++){
    int randchar = rand() % 255;
    bytes1to15[i] = (char)randchar;
  }

  unsigned char (*lambda_chungus)[16] = (unsigned char(*)[16]) malloc(256*16*sizeof(unsigned char));
  for (size_t i=0; i<256; i++){
    unsigned char byte0 = (char)i;
    lambda_chungus[i][0] = byte0;
    for (size_t j=1; j<15; i++){
      lambda_chungus[i][j] = bytes1to15[j];
    }
  }

  return lambda_chungus;
